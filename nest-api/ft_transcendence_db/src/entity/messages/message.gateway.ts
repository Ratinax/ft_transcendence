...
export class MessagesGateway {
  ...
  async create(@ConnectedSocket() client: Socket,
    @MessageBody() body: {sessionCookie: string, channel_id: number, channelName: string, message: string, isAGameInvite: boolean, game: gameOptions | undefined}) 
    {
      if (!body || !body.sessionCookie || !body.channel_id || !body.channelName || body.message === undefined || body.isAGameInvite === undefined)
        return ;
      ...
      this.server.to(body.channelName).except(client.id).emit('addMessage', {message: {content: body.message, dateSent: new Date(Date.now()), channel: message.channel, user: {pseudo: user.pseudo, nickname: user.nickname}, isAGameInvite: message.isAGameInvite}});
